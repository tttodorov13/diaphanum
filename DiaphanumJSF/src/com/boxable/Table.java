...
import com.boxable.text.WrappingFunction;
import com.boxable.utils.FontUtils;
import com.boxable.utils.PDStreamUtils;
...
public abstract class Table<T extends PDPage>
{
     ...
     */
    @Deprecated
    public Table(float yStart, float yStartNewPage, float pageBottomMargin, float width,
            float margin, PDDocument document, T currentPage, boolean drawLines, boolean drawContent)
            throws IOException
    {
        this(yStart, yStartNewPage, 0, pageBottomMargin, width, margin, document, currentPage,
                drawLines, drawContent, null);
    }
     ...
     */
    @Deprecated
    public Table(float yStartNewPage, float pageBottomMargin, float width, float margin,
            PDDocument document, boolean drawLines, boolean drawContent) throws IOException
    {
        this(yStartNewPage, 0, pageBottomMargin, width, margin, document, drawLines, drawContent,
                null);
    }

    public Table(float yStart, float yStartNewPage, float pageTopMargin, float pageBottomMargin,
            float width, float margin, PDDocument document, T currentPage, boolean drawLines,
    ...
    public Table(float yStartNewPage, float pageTopMargin, float pageBottomMargin, float width,
            float margin, PDDocument document, boolean drawLines, boolean drawContent,
    ...
    public void drawTitle(String title, PDFont font, int fontSize, float tableWidth, float height,
            String alignment, float freeSpaceForPageBreak, boolean drawHeaderMargin)
            throws IOException
    {
        drawTitle(title, font, fontSize, tableWidth, height, alignment, freeSpaceForPageBreak,
                null, drawHeaderMargin);
    }

    public void drawTitle(String title, PDFont font, int fontSize, float tableWidth, float height,
            String alignment, float freeSpaceForPageBreak, WrappingFunction wrappingFunction,
            boolean drawHeaderMargin) throws IOException
    {
        ...
        else
        {
            ...
            Paragraph paragraph = new Paragraph(title, font, fontSize, tableWidth,
                    HorizontalAlignment.get(alignment), wrappingFunction);
            ...
            {
                // margin
                PDStreamUtils.rect(tableContentStream, margin, yStart, width, headerBottomMargin,
                        Color.CYAN);
            }
        }
    ...
    public float draw() throws IOException
    {
        ...
        for (Row<T> row : rows)
        {
            if (row == header)
            {
                ...
                float headerHeightIncludingFirstDataRow = header.getHeight()
                        + rows.get(1).getHeight();
    ...
    /**
     * <p>
     * Method to switch between the {@link PageProvider} and the abstract method {@link Table#createPage()}, preferring
     * the {@link PageProvider}.
     * </p>
     * <p>
    ...
    {
        throw new IllegalStateException("You either have to provide a "
                + PageProvider.class.getCanonicalName() + " or override this method");
    }
    ...
    private void drawCellContent(Row<T> row) throws IOException
    {
        ...
        for (Cell<T> cell : row.getCells())
        {
            ...
            for (String line : cell.getParagraph().getLines())
            {
                ...
                float freeSpaceWithinLine = cell.getInnerWidth() - cell.getHorizontalFreeSpace()
                        - tw;
                switch (cell.getAlign())
                {
                case CENTER:
    ...
    private void fillCellColor(Cell<T> cell, float yStart, float xStart,
            Iterator<Cell<T>> cellIterator) throws IOException
    {
        // Fill Cell Color
    ...
    private void endTable() throws IOException
    {
        ...
        {
            // Draw line at bottom
            drawLine("Row Bottom Border ", this.margin, this.yStart, this.margin + width,
                    this.yStart);
        }
        this.tableContentStream.close();
    }
    ...
    public float getHeaderAndDataHeight()
    {
        return header == null ? 0 : header.getHeight()
                + rows.get(header == null ? 0 : 1).getHeight();
    }
